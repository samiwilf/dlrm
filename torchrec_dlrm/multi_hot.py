...
class Multihot():
    def __init__(
        ...
        self.ln_emb = ln_emb
        print("make indices cache")
        self.lS_i_offsets_cache = self.__make_multi_hot_indices_cache(self.multi_hot_sizes, ln_emb)
        print("make offsets cache")
        self.lS_o_cache = self.__make_offsets_cache(self.multi_hot_sizes, ln_emb, batch_size)
        print("CACHES DONE")
    ...
    def __make_multi_hot_indices_cache(
        ...
        cache = [ np.zeros((rows_count, multi_hot_size)) for rows_count, multi_hot_size in zip(ln_emb, multi_hot_sizes) ]
        for k, (e, multi_hot_size) in enumerate(zip(ln_emb, multi_hot_sizes)):
    ...
    def __make_offsets_cache(
        ...
        for cf, (multi_hot_size, table_length) in enumerate(zip(multi_hot_sizes, ln_emb)):
    ...
    def __make_new_batch(
        ...
        for cf, table_length in enumerate(self.ln_emb):
            sparse_ids = lS_i[cf]
            multi_hot_i = torch.nn.functional.embedding(sparse_ids, self.lS_i_offsets_cache[cf])
            multi_hot_i[:,0] = sparse_ids
